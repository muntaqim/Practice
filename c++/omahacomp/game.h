...
template <typename T>
vector<vector<T>> allCombos (vector<T> choices, int numToPick)
{
    vector<vector<T>> ret;
    vector<T> cur;
    allCombosInternal(choices, ret, numToPick, cur, 0);

    return ret;
}

template <typename T>
void allCombosInternal(vector<T> h, vector<vector<T>> &ret, int numToPick, vector<T> &cur, int start)
{
    ...
    {
        cur.push_back(h[i]);
        allCombosInternal(h, ret, numToPick, cur, i + 1);
        cur.pop_back();
    } 
...
private:
    ...
    CombinedGroup getWorstHand(Hand h)
    {
        ...
        allHandCombos = allCombos(h.getCards(), 2);
        allBoardCombos = allCombos(_gI._board.getCards(), 3);
